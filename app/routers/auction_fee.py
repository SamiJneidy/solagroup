...
async def get_auction_fee_by_id(id: int, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
...
async def get_auction_fees(page: int = 1, limit: int = 10, auction: str = Query(None, description="Enter 'COPART' or 'IAAI'"), db: Session = Depends(get_db), current_user = Depends(get_current_user)):
...
async def update_auction_fee(id: int, data: schemas.AuctionFeeUpdate, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
