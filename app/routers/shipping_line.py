...
async def create_shipping_line(data: schemas.ShippingLineCreate, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def update_shipping_line(id: int, data: schemas.ShippingLineUpdate, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def delete_shipping_line(id: int, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
