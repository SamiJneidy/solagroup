...
async def estimate_cost(data: schemas.EstimateCostRequest, db: Session) -> schemas.EstimateCostResponse:
    ...
    total_cost = data.amount + inland_transport_cost + auction_fee_amount + additional_fee + maritime_transport_cost/data.shipping_type
    car_info = await utils.decode_vin(data.vin)
    response = schemas.EstimateCostResponse(cost=total_cost, **car_info.model_dump())
    return response
