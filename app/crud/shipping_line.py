...
async def get_shipping_lines(db: Session, page: int = 1, limit: int = 10) -> list[schemas.ShippingLine]:
    stmt = select(models.ShippingLine).offset((page-1)*limit).limit(limit)
    sources = [schemas.ShippingLine.model_validate(source) for source in db.execute(stmt).scalars().all()]
    return sources
